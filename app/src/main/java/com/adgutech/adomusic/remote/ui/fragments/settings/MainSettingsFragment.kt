...
class MainSettingsFragment : Fragment(), View.OnClickListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        /*binding.deleteAccessButton.apply {
            if (mainActivity.spotifyAppRemote?.isConnected == true) {
                isVisible()
                setOnClickListener {
                    goToSpotifyApplicationsManager()
                }
            } else {
                isGone()
            }
    ...
    /*private fun goToSpotifyApplicationsManager() {
        val intent = Intent(Intent.ACTION_VIEW)
        intent.setData(Uri.parse(SPOTIFY_APPLICATIONS_MANAGER))
        startActivity(intent)
    }*/

    private fun goToSpotifySettings() {
        if (mainActivity.hasSpotifyInstalled()) {
            val intent = Intent(Intent.ACTION_VIEW)
            intent.setData(Uri.parse(SPOTIFY_PREFERENCE))
            startActivity(intent)
        } else {
