...
abstract class AbsSpotifyServiceActivity : AbsBaseActivity(), OnSpotifyServiceEventListener {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_CODE) {
            ...
            when (response.type) {
                 ...
                 */
                ERROR -> {
                    logE("Auth error: ${response.error}")
                }
                /**
                 * Most likely auth flow was cancelled.
                 */
                else -> {
                    logD("Auth result: ${response.type}")
                }
            }
        }
    }
    ...
    fun showNotInternetDialog(callback: () -> Unit) {
        disableScreenRotation()
        val materialDialog = materialDialog(R.string.title_not_internet)
        materialDialog.setMessage(R.string.message_not_internet)
        materialDialog.setPositiveButton(R.string.action_ok) { dialog, _ ->
            enableScreenRotation()
            dialog.dismiss()
            finish()
        }
        materialDialog.setNeutralButton(R.string.action_try_again) { dialog, _ ->
            enableScreenRotation()
            dialog.dismiss()
            callback()
        }
