...
class MusicProgressViewUpdateHelper : Handler {
    ...
    constructor(callback: Callback) : super(Looper.getMainLooper()) {
        ...
        this.intervalPlaying = UPDATE_INTERVAL_PLAYING.toLong()
        this.intervalPaused = UPDATE_INTERVAL_PAUSED.toLong()
    }
    ...
    private fun refreshProgressViews(): Long {
        val progressMillis = AppRemoteHelper.trackProgressMillis
        val totalMillis = AppRemoteHelper.trackDurationMillis
        if (totalMillis > 0) {
            firstUpdate = false
            callback?.onUpdateProgressViews(progressMillis, totalMillis)
        }
        if (!AppRemoteHelper.isPlaying && !firstUpdate) {
            return intervalPaused
        }

        val remainingMillis = intervalPlaying - progressMillis % intervalPlaying

        return max(MIN_INTERVAL.toLong(), remainingMillis)
    }
