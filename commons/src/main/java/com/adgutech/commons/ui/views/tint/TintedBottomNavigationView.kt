...
class TintedBottomNavigationView @JvmOverloads constructor(
    ...
    init {
        if (!isInEditMode) {
            /**
             * If we are in Immersive mode we have to just set empty OnApplyWindowInsetsListener
             * as bottom, start, and end padding is always applied (with the help of
             * OnApplyWindowInsetsListener) to BottomNavigationView to dodge the system
             * navigation bar (so we basically clear that listener).
             */
            if (context.preference.isFullScreenMode) {
            ...
            labelVisibilityMode = context.preference.tabTitleMode
